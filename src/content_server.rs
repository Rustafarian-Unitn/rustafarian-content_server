...
impl ContentServer {
    ...
    fn process_request(
        ...
        match BrowserRequestWrapper::from_string(raw_content) {
            ...
            Err(err) => {
                self.log(
                    format!("Error deserializing request: {}\n", err).as_str(),
                    LogLevel::ERROR,
                );
            }
        }
    ...
    pub fn handle_files_list(&mut self, source_id: NodeId, session_id: u64, route: Vec<u8>) {
        ...
        match self.server_type {
            ...
            ServerType::Chat => {
                self.log(
                    format!("Error: ServerType::Chat is not supported!\n").as_str(),
                    LogLevel::ERROR,
                );
                std::process::exit(1);
            }
            ...
            match fs::read(file_path) {
                ...
                Ok(file_data) => {
                    let file_string = match String::from_utf8(file_data) {
                        ...
                        Err(err) => {
                            self.log(
                                format!("Error converting file data to String: {}\n", err).as_str(),
                                LogLevel::ERROR,
                            );
                            return;
                        }
    ...
    pub fn handle_media_request(
        ...
        if let Some(media_path) = self.media.get(&id) {
            ...
            // If the file with that ID does not exist print error
            self.log(
                format!("Media with ID '{}' not found\n", id).as_str(),
                LogLevel::ERROR,
            );
        }
    }
    ...
    pub fn resend_nacks_in_queue(&mut self) {
        self.log(
            format!("Resending packet for nack\n").as_str(),
            LogLevel::DEBUG,
        );
    ...
    fn resend_packet(
        ...
        fragment_index: u64,
        session_id: u64,
        session_to_remove: &mut Vec<u64>,
        nack_type: NackType,
    ) {
        self.log(
            format!(
                "Resending packet index={:?}, session={:?}\n",
        ...
        if let Some(packets) = self.sent_packets.get_mut(&session_id) {
            if let Some(packet) = packets.iter_mut().find(|p| match &p.pack_type {
                PacketType::MsgFragment(fragment) => fragment.fragment_index == fragment_index,
                _ => false,
            }) {
                println!(
                    "Found packet with session ID: {} and fragment index: {}\n",
                    session_id, fragment_index
                );
                let destination_id =
                    packet.routing_header.hops[packet.routing_header.hops.len() - 1];
                ...
                match self.senders.get(&next_hop) {
                    Some(sender) => {
                        sender.send(forward_packet).unwrap_or_else(|err| {
                            self.log(
                                format!("Failed to forward packet: {}\n", err).as_str(),
                                LogLevel::ERROR,
                            );
                        });
                    }
