    ...
    fn error_routing_test() {
        ...
        if let Some(packets_stored) = server.sent_packets.get(&12345) {
            for pack in packets_stored {
                println!(
                    "server.sent_packet id={:?},index={:?}",
                    pack.session_id,
                    pack.get_fragment_index()
                );
            }
        }
        ...
        let nack1 = Nack {
            fragment_index: 1,
            nack_type: NackType::ErrorInRouting(21),
        };
        ...
        loop {
            match neighbor.1.recv() {
                Ok(received_packet) => match received_packet.pack_type {
                    PacketType::MsgFragment(fragment) => {
                        println!(
                            "Frammento n {} di {} con session_id {}",
